...
mod tests {
    ...
    fn test_fluxa_error_from_configuration_error() {
        let config_error = ServiceConfigurationError::ErrorInConfiguration("test error".to_string());
        let fluxa_error = FluxaError::from(config_error);
        
        assert!(matches!(fluxa_error, FluxaError::Configuration(_)));
        assert_eq!(fluxa_error.to_string(), "Configuration error: Configuration error test error");
    }
    ...
    fn test_service_error_from_monitored_service_error() {
        ...
        assert_eq!(service_error.to_string(), "Monitored service error: invalid is not valid url");
    }
    ...
    fn test_notification_error_send_failed() {
        ...
        assert_eq!(notification_error.to_string(), "Failed to send notification: server error");
    }

    #[test]
    fn test_http_error_addr_parse() {
        let addr_parse_error = "invalid:address:format".parse::<std::net::SocketAddr>().unwrap_err();
